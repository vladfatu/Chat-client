...
class Input extends Thread {
    ...
    public void run() {
        ...
        while (ok) {
            ...
            {
                try {
                    if (ok) mesaj = stream.readUTF();
                } catch (IOException ex) {
                    System.out.println("Socketul a fost deja inchis");
                }
                Login.eroare.setText(mesaj);
                if (mesaj.equals("bine")) a=false;
            }
            try {
                mesaj = stream.readUTF();
                ...
                if (s1.equals("/adauga"))
                {
                    ...
                    if (!s2.equals("prost")) Meniu.m.adauga(s2,st.nextToken());
                    else JOptionPane.showMessageDialog(Meniu.m,"Nu exista nici un utilizator cu aceasta porecla","Eroare",JOptionPane.ERROR_MESSAGE);
                ...
                else if (s1.equals("/transfer"))
                {
                    ...
                    int k;
                    while( ((k=stream.read()) != -1)&& k!=0) fos.write(k);
                ...
                if (ok) System.out.println("[eroare1]: " + e.getMessage());
            }
        }
    }
}
